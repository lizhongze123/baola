...
public class MyDateUtils extends DateUtils {
    ...
    public static long calculateDifferentSecond(long startTimeMillis,
                                                long endTimeMillis) {
        return calculateDifference(startTimeMillis, endTimeMillis,
                DifferenceMode.Second);
    }
    ...
    public static long calculateDifference(long startTimeMillis,
                                           long endTimeMillis, DifferenceMode mode) {
        return calculateDifference(new Date(startTimeMillis), new Date(
                endTimeMillis), mode);
    }
    ...
    public static long[] calculateDifference(long differentMilliSeconds) {
        ...
        // differentMilliSeconds, elapsedDays, elapsedHours, elapsedMinutes,
        // elapsedSeconds));
    ...
    /**
     * 功能：判断日期是否和当前date对象在同一天。 参见：http://www.cnblogs.com/myzhijie/p/3330970.html
     *
     * @param date 比较的日期
    ...
    public static boolean isSameDay(Date date) {
        ...
        return (nowCalendar.get(Calendar.ERA) == newCalendar.get(Calendar.ERA)
                && nowCalendar.get(Calendar.YEAR) == newCalendar
                .get(Calendar.YEAR) && nowCalendar
                .get(Calendar.DAY_OF_YEAR) == newCalendar
