...
import com.XMBT.bluetooth.le.bean.AixsPoint;
import com.XMBT.bluetooth.le.bean.ChartAttrs;
 ...
 */
public class Line2ChartView extends View implements View.OnTouchListener {

    private static final String TAG = Line2ChartView.class.getSimpleName();
    ...
    //X轴的节点集合
    private List<AixsPoint> xAixsPoints = new ArrayList<>(0);
    //折线图的列表
    ...
    public Line2ChartView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public Line2ChartView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setOnTouchListener(OnTouchListener l) {
        super.setOnTouchListener((l instanceof Line2ChartView) ? l : null);
    }
    ...
    public void drawDataPoint(Canvas canvas, ChartLine chartLine) {
        ...
        final List<DataAixsPoint> dataPoints = chartLine.getDataPoints();

        final Path path = new Path();
        final Path strokePath = new Path();
    ...
    public void drawAixsPoint(AixsPoint aisPoint, Canvas canvas, Paint paint, List<AixsPoint> aisPoints) {
        ...
        if (aisPoints.size() == 301) {
            ...
            if (aisPoint.getTitle().equals("0.0")) {
                canvas.drawText(time, targetRect.centerX(), baseline, paint);
            } else if (aisPoint.getTitle().equals("1.0000000000000004")) {
                Time curtime1 = new Time(System.currentTimeMillis() + 60000);
                String time1 = sdf.format(curtime1);
                canvas.drawText(time1, targetRect.centerX(), baseline, paint);
            } else if (aisPoint.getTitle().equals("2.0000000000000013")) {
                Time curtime2 = new Time(System.currentTimeMillis() + 120000);
                String time2 = sdf.format(curtime2);
                canvas.drawText(time2, targetRect.centerX(), baseline, paint);
            } else if (aisPoint.getTitle().equals("3.000000000000002")) {
                Time curtime2 = new Time(System.currentTimeMillis() + 180000);
                String time2 = sdf.format(curtime2);
                canvas.drawText(time2, targetRect.centerX(), baseline, paint);
            } else if (aisPoint.getTitle().equals("4.000000000000003")) {
                Time curtime2 = new Time(System.currentTimeMillis() + 240000);
                String time2 = sdf.format(curtime2);
                canvas.drawText(time2, targetRect.centerX(), baseline, paint);
            } else if (aisPoint.getTitle().equals("4.999999999999981")) {
                Time curtime2 = new Time(System.currentTimeMillis() + 300000);
                String time2 = sdf.format(curtime2);
                canvas.drawText(time2, targetRect.centerX(), baseline, paint);
            } else if (aisPoint.getTitle().equals("5.99999999999996")) {
                Time curtime2 = new Time(System.currentTimeMillis() + 360000);
                String time2 = sdf.format(curtime2);
                canvas.drawText(time2, targetRect.centerX(), baseline, paint);
            }
        } else {
...
//        Log.i(TAG,"touchX="+touchX+" ; K="+k+" ; b="+b);
     ...
     * @param titles 标题集合
     */
    public Line2ChartView updateXAixsTitles(List<String> titles) {
        xAixsPoints.clear();
     ...
     */
    public Line2ChartView updateYAixsTitles(double maxYAixsVal, int yAixsStepSize) {
        this.maxYAixsVal = maxYAixsVal;
        this.yAixsStepSize = yAixsStepSize;
        return this;
    }

    public Line2ChartView updateYAuxAixsTitles(double maxYAuxAixsVal) {
